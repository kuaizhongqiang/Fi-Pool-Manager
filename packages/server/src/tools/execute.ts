 ...
 */
export async function runLocalAnalysis(code: string) {
  try {
 ...
 * @returns { success: true, data: { date: string } }
 */
export async function runFullPipeline(code: string) {
  try {
 ...
 */
export async function runPoolAnalysis(poolId: number) {
  try {
 ...
 * @returns { success: true, data: { total: number } }
 */
export async function runPoolFullPipeline(poolId: number) {
  try {
    const stocks = await poolService.getPoolStocks(poolId);
    let completed = 0;
    for (const s of stocks) {
