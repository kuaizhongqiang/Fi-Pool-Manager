...
在 `run_pool_full_pipeline`（股池完整流水线）执行完成后**自动触发**。
...
1. 收集数据
   → 遍历所有股池
   → 每只股票读取最新的 daily_analysis_report（技术信号 + 摘要）
   → 读取最新的 final_report（综合结论）
   → 汇总看多/看空/中性信号统计
2. 构建 LLM prompt
   → 将所有股池的结构化数据格式化
   → 要求 LLM 生成 800-1200 字的每日投资综述
3. 调用 LLM
   → 单次调用，temperature=0.5, maxTokens=2000
   → 使用独立 session
4. 输出到控制台
   → 分隔线 + 统计 + LLM 综述
```
